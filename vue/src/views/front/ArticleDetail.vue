<template>
  <div>
    <div style="width:1100px;margin: 5px auto;color: white">
      <el-card>
        <div style="margin: 10px 0; ">
          <div class="pd-10" style="font-size: 20px; font-weight:bolder;color: #3F5EFB; cursor: pointer">{{
              article.header
            }}
          </div>
          <div style="font-size: 15px; margin-top: 12px">
            <i class="el-icon-user-solid"></i> <span>{{ article.user }}</span>
            <i class="el-icon-time" style="margin-left: 15px"></i> <span>{{ article.time }}</span>
          </div>
        </div>
      ...
      <div style="margin: 20px 0">
        <div style="margin: 10px 0">
          <div style="border-bottom: 1px solid pink; padding: 13px 0; font-size: 20px;font-weight: bolder">评论</div>
          <div style="padding: 10px 0">
            <el-input size="small" type="textarea" v-model="commentForm.content"></el-input>
          </div>
          <div class="pd-10" style="text-align: right">
            <el-button type="primary" size="small" @click="save" style="border-radius: 60%">评论</el-button>
          </div>
        </div>
      </div>
          ...
          <div v-for="item in comments" :key="item.id" style="border-bottom: 1px solid #ccc; padding: 10px 0; ">
            <div style="display: flex">
              ...
              <div style="flex: 1; font-size: 14px; padding: 5px 0; line-height: 25px">
                <b>{{ item.nickname }}：</b>
                <span style="padding-left: 5px">{{ item.content }}</span>

                <div style="display: flex; line-height: 20px; margin-top: 10px">
                  ...
                  <div style="text-align: right; flex: 1">
                    ...
                    <el-popconfirm
                        ...
                        @confirm="del(item.id)"
                    >
                      <el-button slot="reference" type="text" style="color: red" v-if="user.id === item.userId">删除
                      </el-button>
                    </el-popconfirm>
            ...
            <div v-if="item.children.length" style="padding-left: 100px;">
              <div v-for="subItem in item.children" :key="subItem.id"
                   style="background-color: #f0f0f0; padding: 5px 20px;border-radius: 8px">
                <!--          回复列表-->
                <div style="font-size: 14px; padding: 5px 0; line-height: 25px">
                  <div>
                    <b style="color: #3a8ee6" v-if="subItem.puserNickname">@{{ subItem.puserNickname }}</b>
                  </div>
                  <div style="padding-left: 5px;display: flex">
                    <div style="width: 35px; text-align: center;display: inline-block">
                      <el-image :src="subItem.avatar" style="width: 35px; height: 35px; border-radius: 50%"></el-image>
                    </div>
                    <div style="flex: 1;margin-top: 8px">
                      <b style="margin-left: 10px;">{{ subItem.nickname }}：</b>
