<template>
  <div>
    <div style="margin: 10px 0">
      <el-input style="width: 200px" placeholder="请输入标题" suffix-icon="el-icon-search" v-model="header"></el-input>
    ...
    <el-dialog title="文章信息" :visible.sync="dialogFormVisible" width="60%" >
      <el-form label-width="80px" size="small">
        <el-form-item label="文章标题">
          <el-input v-model="form.header" autocomplete="off"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    delBatch() {
      ...
      this.request.post("/article/del/batch", ids).then(res => {
        if (res.code === '200') {
          this.$message.success("文章批量删除成功...")
          this.load()
        } else {
          this.$message.error("文章批量删除失败!")
        }
      })
    },
    save() {
